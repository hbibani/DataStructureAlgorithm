...
private:
	...
	NODE<TYPE>* _retrieveV2(KTYPE        key,
		NODE<TYPE>* root);
...
public:
	...
	bool  AVL_Update(KTYPE  key, TYPE dataOut); // dongmo
	bool  AVL_Empty(void);
	bool  AVL_Full(void);
	int   AVL_Count(void);
	bool  AVL_InsertV2(Index relevantIDs);
	bool  AVL_RetrieveV2(KTYPE   key, TYPE& dataOut);
...
/*	========================================
	This function inserts new data into the tree or updates existing data.
 Pre    dataIn contains data to be inserted
 Post   data have been inserted or updated if already exist or memory overflow
 Return success (true) or overflow (false)
 */

template <class TYPE, class KTYPE>
bool   AvlTree<TYPE, KTYPE> ::AVL_InsertV2(Index strucIndex)
{
	//	Local Definitions
	NODE<TYPE>* newPtr;
	NODE<TYPE>* node = NULL;
	...
	node = _retrieveV2(strucIndex.key, tree);//retrieve info if present
	if (node != NULL) 
	{
		TYPE& nodeCapture = node->data;

		for (int i = 0; i < strucIndex.idList.size(); i++) 
		{
			bool test = false;
			//make sure no doubles are included
			for (int j = 0; j < nodeCapture.idList.size(); j++)
			{
				if (strucIndex.idList[i] == nodeCapture.idList[j])
				{
					test = true;
				}
			}
			...
			if (test == false) 
			{ // updates idList if key exists
				nodeCapture.idList.push_back(strucIndex.idList[i]);
				node->data = nodeCapture;
			}
		}
	}
	else
	{ // inserts new key & data into tree

		Index newIndex;
		newIndex.key = strucIndex.key;

		for (int i = 0; i < strucIndex.idList.size(); i++)
		{
			newIndex.idList.push_back(strucIndex.idList[i]);
		}
...
::AVL_Retrieve(KTYPE   key, TYPE& dataOut)
{
	...
	node    = _retrieve (key, tree); //CHANGED
...
template <class TYPE, class KTYPE>
bool   AvlTree<TYPE, KTYPE>
::AVL_RetrieveV2(KTYPE   key, TYPE& dataOut)
{
	//	Local Definitions
	NODE<TYPE>* node;
...
template <class TYPE, class KTYPE>
NODE<TYPE>* AvlTree<TYPE, KTYPE>
::_retrieveV2(KTYPE       key,
	NODE<TYPE>* root)
{
	if (root)
	{
		if (key.compare(root->data.key) < 0)
			return _retrieveV2(key, root->left);
		else if (key.compare(root->data.key) > 0)
			return _retrieveV2(key, root->right);
		else
			//Found equal key
			return (root);
	} // if root
...
::AVL_Traverse(void (*process)(TYPE  dataProc)) //changing it for deletion
{
	//	Statements
